...
function ForgetPasswordOTP({ navigation, route }) {
    const [newRandom, setNewRandom] = React.useState(null);
    const [random, setRandom] = React.useState(route.params.random);
    ...
    const VerifyNumber = (val) => {
        if (val == random || val == newRandom) {
            navigation.navigate('ResetPassword', { number: route.params.number })
        } else {
            val = null
    ...
    const ResendCode = () => {
        // alert('your code has been sent successfully!')
        setCounter(60)
